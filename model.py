...
def read_data():
    ...
    for sub_dir_name in listdir(DATA_ROOT_PATH):
        ...
        for image_center, image_left, image_right, angle in read_data_from_one_dir(sub_dir):
            images.extend([image_center, image_left, image_right, np.fliplr(image_center), np.fliplr(image_left), np.fliplr(image_right)])
            angles.extend([angle, angle + CORRECTION, angle - CORRECTION, -angle, -angle - CORRECTION, -angle + CORRECTION])
...
def lenet():
    model = Sequential()
    model.add(Cropping2D(cropping=((70, 25), (0, 0)), input_shape=(160, 320, 3))) # 55x320x3
    model.add(Lambda(lambda x: x / 255 - 0.5))
    model.add(Convolution2D(6, 5, 5, activation='relu')) # 51x316x6
    model.add(MaxPooling2D()) # 25x153x6
    model.add(Convolution2D(6, 5, 5, activation='relu')) # 21x149x16
...
def nvidia():
    ...
    model.add(Lambda(lambda x: x / 255 - 0.5))
    model.add(Convolution2D(24, 5, 5, subsample=(2, 2), activation='relu')) # 31x158x24
    model.add(Convolution2D(36, 5, 5, subsample=(2, 2), activation='relu')) # 14x77x36
    model.add(Convolution2D(48, 5, 5, subsample=(2, 2), activation='relu')) # 5x37x48
